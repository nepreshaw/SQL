...
SELECT VendorName, InvoiceNumber, InvoiceDate,
	InvoiceTotal - PaymentTotal - CreditTotal AS Balance --selecting the 4 columns to display
FROM Vendors JOIN Invoices --states which tables to be joined 
	ON Vendors.VendorID = Invoices.VendorID --sates the join condition 
WHERE InvoiceTotal - PaymentTotal - CreditTotal > 0 --states which rows to include
ORDER BY VendorName 
